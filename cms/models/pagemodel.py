...
class Page(MpttPublisher):
    ...
    def get_reverse_id(self):
        """
        every page can be found using cms template tags that require a
        reverse_id, even if one is not set.  Just use page.get_reverse_id
        instead of a hardcoded reverse_id
        """
        if self.reverse_id:
            return self.reverse_id
        else:
            return 'page_%d' % self.id
